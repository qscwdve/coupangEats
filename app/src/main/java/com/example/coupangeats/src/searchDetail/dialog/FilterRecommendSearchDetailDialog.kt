package com.example.coupangeats.src.searchDetail.dialog
...
class FilterRecommendSearchDetailDialog(val activity: SearchDetailActivity, val version: Int): BottomSheetDialogFragment() {
    ...
    private fun finishResult(option : Int) {
        if(option != select){
            activity.changeRecommendFilter(selectStingText[option - 1], selectString[option - 1], option)
            dismiss()
        }
    }
}