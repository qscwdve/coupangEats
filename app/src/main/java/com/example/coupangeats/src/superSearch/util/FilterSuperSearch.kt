...
class FilterSuperSearch (val superSearchActivity: SuperSearchActivity, val version: Int, val select: Int): BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.dialogFilterApply.setOnClickListener {
            ...
            if(version == 1){
                // 배달비 순
                superSearchActivity.changeDeliveryFilter(priceDeliveryArray[mSelect - 1], mSelectString, mSelect)
            } else {
                // 최소 주문 순
                superSearchActivity.changeOrderMinFilter(priceMinorderArray[mSelect - 1], mSelectString, mSelect)
            }
            dismiss()
        }
        // 선택시 그림 밑 숫자 바꾸기
